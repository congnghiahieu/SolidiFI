...
def convert_stack_value_to_int(stack_value):
    if stack_value[0] == int:
        ...
        raise Exception(
            "Error: Cannot convert stack value to int. Unknown type: "
            + str(stack_value[0])
        )
...
def convert_stack_value_to_hex(stack_value):
    if stack_value[0] == int:
        ...
        raise Exception(
            "Error: Cannot convert stack value to hex. Unknown type: "
            + str(stack_value[0])
        )
...
def print_individual_solution_as_transaction(
    logger,
    individual_solution,
    color="",
    function_signature_mapping={},
    ...
    for index, input in enumerate(individual_solution):
        ...
        if not transaction["to"] == None:
            ...
            if len(individual_solution) == 1 or (
                transaction_index != None and transaction_index == 0
            ):
                if hash in function_signature_mapping:
                    logger.title(
                        color
                        + "Transaction - "
                        + function_signature_mapping[hash]
                        + ":"
                    )
                else:
