...
def analysis_depend_contract(
    file_path: str, _contract_name: str, _solc_version: str, _solc_path
) -> (Tuple)[List, Slither]:
    res = set()  # 需要被部署的合约
    ...
    while not to_be_deep_analysis.empty():
        ...
        for v in contract.all_state_variables_written:
            if (
                ...
                logger.debug(
                    "通过分析合约内被写入的状态变量, 发现依赖的合约: {}".format(
                        v.type.type.name
                    )
                )
        for f in contract.functions:
            ...
            for v in f.variables_written:
                if (
                    ...
                    res.add(v.type.type.name)
                    logger.debug(
                        "通过分析函数的写入变量(局部和状态都算), 发现依赖的合约: {}".format(
                            v.type.type.name
                        )
                    )
    ...
    for depend_c in res.copy():
        if (
            compilation_unit.bytecode_runtime(depend_c) == ""
            or compilation_unit.bytecode_runtime(depend_c) == ""
        ):
...
def analysis_main_contract_constructor(
    file_path: str, _contract_name: str, sl: Slither = None
):
    if sl is None:
    ...
    for (
        ...
        if isinstance(exps, AssignmentOperation):
            ...
            elif isinstance(exps_right, TypeConversion) and isinstance(
                exps_left, Identifier
            ):
                param_name, param_map_contract_name = extract_param_contract_map(
                    exps_right
                )
