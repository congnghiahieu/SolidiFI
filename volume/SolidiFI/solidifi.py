    ...
    for bug_forms in ("s", "f"):
        ...
        for loc in reversed(BIP):
            ...
            if (
                ...
                loc["name"] in ["Block", "FunctionDefinition", "ModifierDefinition"]
                and (new_loc[1] not in BugLog)
                and (eoffset not in injected_loc_src_mapping)
            ):
                inject_file.update(
                    cur_contr_file, new_loc[1] + 2, b"\n" + bug_snip.strip()
                )
                BugLog = inject_file.adjust_injected_loc(
                    BugLog, new_loc[2] + 2, bug_snip_len
                )
                BugLog.append(
                    {
                        "loc": new_loc[2] + 1,
...
def get_bug_info(bug_type):
    ...
    bug_type_details = [
        bug_info for bug_info in bug_types if bug_info["bug_type"] == bug_type
    ]
...
def get_potential_locs(ast, bug_snip_type):
    ...
    block_list = [
        ...
        for name in all_locs
        if name["name"]
        in ("FunctionDefinition", "ModifierDefinition", "EventDefinition", "Block")
    ]
    ...
    interface_locs.extend(
        inject_file.get_pattern_all_offsets(src_contr_file, "library")
    )
    interface_list = [
    ...
    for stm_type in stm_types:
        ...
        if stm_type in ("VariableDeclaration", "Identifier"):
            for vr in sub_stm:
                if (
                    not within_interface(
                        interface_list,
                        interface_locs,
                        int(get_src(vr["src"])["eoffset"]),
                    )
                    and not is_paramter(param_list, int(get_src(vr["src"])["eoffset"]))
                    and not within_struct_block(
                        struct_list, int(get_src(vr["src"])["eoffset"])
                    )
                ):
                    stmt_locs.append(vr)
            ...
            for stm in sub_stm:
                if not within_interface(
                    interface_list, interface_locs, int(get_src(stm["src"])["eoffset"])
                ) and not within_main_block(
                    block_list, int(get_src(stm["src"])["eoffset"])
                ):
...
def code_transform(filename, bug_type):
    ...
    for config in code_trans_configs.sections():
        ...
        existing_patterns.append(
            {"bug_type": _bug_type, "sec_snip": _sec_snip, "bug_snip": _bug_snip}
        )
    ...
    with open(filename, "r+b") as myfile:
        ...
        for bug in bug_patterns:
            ...
            if len(bug_locs) > 0:
                ret = re.sub(
                    re.compile(bug["sec_snip"].encode()), bug["bug_snip"].encode(), ret
                )
                myfile.seek(0, 0)
                myfile.write(ret)
                myfile.truncate()
...
def weaken_sec_mec(filename, bug_type):
    ...
    for bug in bug_patterns:
        ...
        bug_locs = inject_file.get_pattern_all_offsets(
...
def main(argv=None):
    ...
    try:
        ...
        if argv[1] in ("--inject", "-i"):
            ...
            ast_json_file = os.path.join(
                ast_json_files_dir, os.path.splitext(tail)[0] + ".json"
            )
            ...
            csv_file = os.path.join(
                buggy_dir, "BugLog_" + tail[0 : len(tail) - 4] + ".csv"
            )
